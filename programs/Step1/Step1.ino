...
void loop() {
  ...
  if (delayTime > 25000){ //after 25 secs, restart the trial by retracting the choice ports for 10 s
  ...
  if (leftSens < 0.5){ //potential poke (1st reading may be innaccurate, bouncing)
    Serial.print("Left ");
    delay(200);
    delayTime = delayTime + 200;
    leftSens = digitalRead(leftSensor);
    if (leftSens < 0.5){ //check again to confirmed poke (debouncing)
